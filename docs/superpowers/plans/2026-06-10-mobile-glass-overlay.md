...
2. **Reroll Lore** mirrors the desktop `clear-picks` action (`src/drawer/drawer-events.js:234`): `resetAiSearchCache()`, `setLastInjectionSources(null)`, clear `chat_metadata.deeplore_injection_log`, save metadata. All exports confirmed in `src/state.js` (lines 152–153).
...
Add `suppressNextAgenticLoop` to the `../state.js` import list (the rest of the reroll/skip imports come in Task 6; adding them all now is also fine: `resetAiSearchCache`, `setLastInjectionSources`, `setSuppressNextAgenticLoop`).
...
test('mobile quick actions: reroll clears the AI search cache and injection sources', async () => {
    const dom = installMobileDom({ viewportWidth: 390 });
    try {
        aiSearchCache.hash = 'stale-hash';
        setLastInjectionSources([{ title: 'Stale Entry' }]);
        const root = createMobileShell({ buildIndex: async () => {} });
        clickMobileElement(root, '[data-dle-mobile-action="quick-reroll"]');
        await new Promise(resolve => setTimeout(resolve, 0)); // let the async handler settle
        assertEqual(aiSearchCache.hash, '', 'reroll should reset the AI search cache');
        assert(lastInjectionSources === null || lastInjectionSources.length === 0, 'reroll should clear injection sources');
        destroyMobileShell();
    } finally {
        dom.restore();
    }
...
- Header status tap expands the metrics grid; gear opens DLE settings; X closes
