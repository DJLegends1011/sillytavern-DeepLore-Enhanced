...
1. **Nested scrollbars, no gesture support.** Up to **3 independent vertical scrollers** stacked (measured on Filters tab: `#deeplore-panel.dle-overlay-mode` -> `.scrollableInner.dle-drawer-inner` -> `.dle-gating-fields-container`; Browse added `.dle-browse-list`). The overlay fix keeps `.dle-drawer-inner` as a bounded, non-scrolling flex column and makes each active `.dle-tab-panel` the one touch viewport. In overlay mode Browse follows the same rule: its virtual list is a non-scrolling positioning surface inside the Browse tab panel. Inline desktop keeps the branch's existing `.dle-drawer-inner` viewport; mode-specific selectors choose viewport math while a shared target selector handles events and resets across resize. The scroller uses `touch-action: pan-y`, `overscroll-behavior: contain`, and `-webkit-overflow-scrolling: touch`.
