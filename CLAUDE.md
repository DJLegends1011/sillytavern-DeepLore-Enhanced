...
Mobile uses a hybrid dock + drill-in pattern: floating dock as entry point, bottom sheet for local views, slash commands for mature desktop tools when that's the better surface.

Mobile activation: SillyTavern mobile flag, viewport <= 768px, coarse pointer <= 1024px, or localStorage force/disable overrides.
...
Tests live in `test/` as `.mjs` files. Mobile tests are in `test/mobile-ui.test.mjs`.
